...
function ResultBadge({ result }: { result: Game['result'] }) {
  if (result === 'win')  return <span className="text-xs px-1.5 py-0.5 rounded bg-green-900/50 text-green-400 font-bold">勝</span>
  if (result === 'loss') return <span className="text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400 font-bold">負</span>
  return <span className="text-xs px-1.5 py-0.5 rounded bg-yellow-900/40 text-yellow-400 font-bold">分</span>
}

// 強調スタット（打率・OPS等）用セル
function HighlightStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="flex flex-col items-center py-5 px-2 bg-night-750 dark:bg-night-750">
...
function StatRow({ left, right }: {
  ...
  return (
    <div className="grid grid-cols-2 divide-x divide-night-700 odd:bg-night-800 even:bg-night-750/60">
      <div className="flex items-center justify-between px-5 py-3.5">
        <span className="text-sm text-night-300">{left.label}</span>
        <span className="text-xl font-bold text-white">{left.value}</span>
      </div>
      {right ? (
        <div className="flex items-center justify-between px-5 py-3.5">
          <span className="text-sm text-night-300">{right.label}</span>
          <span className="text-xl font-bold text-white">{right.value}</span>
        </div>
      ) : <div />}
    </div>
  )
...
// 共通スタイル
const CARD = 'bg-night-800 rounded-xl shadow-sm border border-night-600 overflow-hidden'
const TH   = 'bg-night-750 text-night-300 text-xs font-medium'
const TD   = 'text-white'
const TD2  = 'text-night-300'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-crimson-400">成績</h1>
        <select
          value={season}
          onChange={(e) => setSeason(parseInt(e.target.value))}
          className="border border-night-600 rounded-lg px-3 py-1.5 text-sm bg-night-800 text-white focus:outline-none focus:ring-2 focus:ring-crimson-400 transition-shadow duration-150"
        >
            ...
            <button
              ...
              className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap border-b-2 -mb-px outline-none transition-colors duration-150 ${
                tab === key
                  ...
                  : 'text-night-400 hover:text-night-200 border-transparent'
              }`}
            >
              {label}
            </button>
      ...
      {loading ? (
        <div className="space-y-3 min-h-[520px]">
          {[1,2,3].map(i => (
            <div key={i} className="bg-night-800 rounded-xl border border-night-600 p-6 animate-pulse">
              <div className="h-4 bg-night-700 rounded w-1/4 mb-4" />
              <div className="grid grid-cols-4 gap-3">
                {[1,2,3,4].map(j => <div key={j} className="h-10 bg-night-700 rounded" />)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4 min-h-[520px]" style={{ opacity: tabVisible ? 1 : 0, transition: tabVisible ? 'opacity 0.14s ease-out' : 'none' }}>
            ...
            <div className="space-y-4">
              ...
              {games.length > 0 && (
                <div className={CARD + ' p-5'}>
                  <h2 className="text-xs font-semibold text-night-300 uppercase tracking-wide mb-3">チーム戦績</h2>
                  <div className="grid grid-cols-5 gap-2 text-center">
                    <div><div className="text-2xl font-bold text-white">{games.length}</div><div className="text-xs text-night-400 mt-0.5">試合</div></div>
                    <div><div className="text-2xl font-bold text-green-400">{wins}</div><div className="text-xs text-night-400 mt-0.5">勝</div></div>
                    <div><div className="text-2xl font-bold text-red-400">{losses}</div><div className="text-xs text-night-400 mt-0.5">負</div></div>
                    <div><div className="text-2xl font-bold text-yellow-400">{draws}</div><div className="text-xs text-night-400 mt-0.5">分</div></div>
                    <div><div className="text-2xl font-bold text-white">{winRate}</div><div className="text-xs text-night-400 mt-0.5">勝率</div></div>
                  </div>
                </div>
              )}
              ...
              <div className={CARD}>
                <div className="px-5 py-3.5 border-b border-night-700">
                  <h2 className="text-xs font-semibold text-night-300 uppercase tracking-wide">打撃成績</h2>
                </div>
                {allAtBats.length === 0 ? (
                  <p className="text-night-400 text-center py-10">まだ打席記録がありません</p>
                ) : (
                  <>
                    {/* 主要KPI */}
          ...
          {tab === 'per-game' && (
            <div className={CARD}>
              {games.length === 0 ? (
                <div className="p-12 text-center text-night-400">試合データがありません</div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-night-700">
                      {games.map((game) => {
                        ...
                        return (
                          <tr key={game.id} className="hover:bg-night-750 transition-colors duration-100">
                            ...
                            <td className="px-3 py-3 text-center font-bold text-crimson-400 text-base">{fmtAvg(gs.avg)}</td>
          ...
          {tab === 'log' && (
            <div className={CARD}>
              {allAtBats.length === 0 ? (
                <div className="p-12 text-center text-night-400">打席データがありません</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                    ...
                    <tbody className="divide-y divide-night-700">
                      {games.flatMap((game) =>
                        game.at_bats.map((ab) => (
                          <tr key={ab.id} className="hover:bg-night-750 transition-colors duration-100">
                            ...
                            <td className="px-3 py-2.5 text-center">
                              {(() => {
                                ...
                                return cnt > 0
                                  ? <span className="text-orange-400 font-bold">{cnt}</span>
                                  : <span className="text-night-600">-</span>
                              })()}
                            </td>
                            <td className="px-3 py-2.5 text-center">
                              {ab.is_run ? <span className="text-blue-400 font-bold">●</span> : <span className="text-night-600">-</span>}
                            </td>
                          </tr>
                        ))
                      )}
          ...
          {tab === 'pitching' && (
            <div className="space-y-4">
              {pitchingStats.length === 0 ? (
                <div className={CARD + ' p-12 text-center text-night-400'}>投手成績が登録されていません</div>
              ) : (
                <>
                  <div className={CARD}>
                    <div className="px-5 py-3.5 border-b border-night-700">
                      <h2 className="text-xs font-semibold text-night-300 uppercase tracking-wide">シーズン投手成績</h2>
                    </div>
                    {/* 主要KPI */}
                    <div className="grid grid-cols-4 divide-x divide-night-700 border-b border-night-700">
                  ...
                  <div className={CARD}>
                    <div className="px-5 py-3.5 border-b border-night-700">
                      <h2 className="text-xs font-semibold text-night-300 uppercase tracking-wide">試合別投手成績</h2>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                        ...
                        <tbody className="divide-y divide-night-700">
                          {games.map((game) => {
                            ...
                            const rColor: Record<string, string> = {
                              win: 'text-green-400 font-bold', loss: 'text-red-400 font-bold',
                              save: 'text-blue-400 font-bold', hold: 'text-purple-400 font-bold', none: 'text-night-400'
                            }
                            return (
                              <tr key={game.id} className="hover:bg-night-750 transition-colors duration-100">
                                <td className={`px-4 py-3 ${TD2}`}>{formatDate(game.game_date)}</td>
          ...
          {tab === 'direction' && (
            allAtBats.length === 0 ? (
              <div className={CARD + ' p-12 text-center text-night-400'}>打席データがありません</div>
            ) : (
